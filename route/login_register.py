...
def login_register_2(conn):
    ...
    if flask.request.method == 'POST': 
        ...
        if re.search('(?:[^A-Za-z0-9_])', flask.request.form.get('id', None)) or re.search('^[0-9_]', flask.request.form.get('id', None)) or flask.request.form.get('id', None) == 'namubot' or len(flask.request.form.get('id', None)) < 3 or len(flask.request.form.get('id', None)) > 32:
        ...
        if sql_data and sql_data[0][0] != '':
            ...
            if first == 0:
                return redirect('/regc/' + flask.request.form.get('id', None))
            else:
                return redirect('/regc/' + flask.request.form.get('id', None))
    else:        
        contract = ''
