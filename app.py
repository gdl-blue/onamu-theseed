...
dislay_set_key = ['호스트 주소', '폿트', '언어', '문법', '암호화 방식']
...
curs.execute('insert into leq (l, m) values (?, ?)', ['0', '0'])
...
@app.route('/new_edit_request/', methods=['POST', 'GET'])
def nerq(name):
...
@app.route('/unstar/<name>')
def unstar(name = None):
    return unstar_2(conn, name)

@app.route('/starred_documents')
def stardoc():
...
@app.route('/topic/<everything:name>/<regex("close|agree|eqclose"):tool>', methods=['GET'])
...
@app.route('/ipacl', methods=['POST', 'GET'])
...
@app.route('/admin/grant/<name>', methods=['POST', 'GET'])
@app.route('/grant/<name>', methods=['POST', 'GET'])
def give_admin(name = None):
    return give_admin_2(conn, name)
    
@app.route('/admin/grant')
@app.route('/grant')
def give_admin_us():
    return give_admin_un_2(conn)
...
@app.route('/contribution/author/<name>/discuss')
@app.route('/contribution/ip/<name>/discuss')
def list_user_topic(name = None):
    return list_user_topic_2(conn, name)

@app.route('/recent_changes')
@app.route('/RecentChanges')
#contribution/author/zqktlwi4fecvo6ri/document
@app.route('/<regex("record"):tool>/<name>')
@app.route('/<regex("contribution"):tool>/author/<name>/document')
@app.route('/<regex("contribution"):tool>/ip/<name>/document')
